...
{
	int16_t GPIO_MOSI = CONFIG_MOSI_GPIO;
	int16_t GPIO_SCLK = CONFIG_SCLK_GPIO;
	int16_t GPIO_CS = CONFIG_CS_GPIO;
	int16_t GPIO_DC = CONFIG_DC_GPIO;
	int16_t GPIO_RESET = CONFIG_RESET_GPIO;
	int16_t GPIO_BL = CONFIG_BL_GPIO;
	esp_err_t ret;
...
uint16_t alphablend(uint16_t fg, uint16_t bg, uint8_t alpha)
{
	...
	return (uint16_t)((
												((alpha * (uint32_t)(fg & MASK_RB) + beta * (uint32_t)(bg & MASK_RB)) & MASK_MUL_RB) |
...
{
	_lcdDrawInFramePixel(dev, x, y, alphablend(color, lcdInFramePixel(dev, x, y), alpha));
}
